...
int prepare_statements() {
     string sql = "INSERT INTO logs (data) VALUES (?);";
...
static int logs_callback(void* data, int argc, char** argv, char** azColName) {
    if (argc == 3) {
         cout << argv[0] << ". (" << argv[2] << ") " << argv[1] <<  endl;
    }
    else {
         cout << "Error: Expected 3 columns, got " << argc <<  endl;
    }
...
int list_logs() {
    char* err_msg = nullptr;
     string sql = "SELECT id, datetime(datetime,'localtime'), data FROM logs";
